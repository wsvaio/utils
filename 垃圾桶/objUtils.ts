...
export function assign(obj1, obj2, deep = 1) {
  ...
  const dels = Object.keys(obj1).filter(item => !obj2Keys.includes(item));
...
export function merge(obj1, obj2, { deep = 1, overwrite = true, del = false } = {}) {
  deep--;
  const dels = Object.keys(obj1).filter(item => !Object.keys(obj2).includes(item));
  if (del) for (const key of dels) delete obj1[key];
  for (const [key, val] of Object.entries(obj2)) {
